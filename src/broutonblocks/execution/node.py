...
class Handler:
    def __init__(self,
                 ...
                 info,
                 function):
        self.declaration = nodetype.HandlerDeclaration(name,
                                                       type_,
                                                       receives_multiple,
                                                       info)
...
class Node:
    ...
    def wait(self):
        for processor in [self.initialize_processor,
    ...
    def close_all_sockets(self):
        for socket in [self.sub_socket, self.pub_socket, self.service_socket]:
            try:
                ...
                print('Trying to close down socket: '
    ...
    def fire(self, event, value, msg_id=None):
        try:
            ...
            prefix = "{id}|{event} "\
                .format(id=self.id, event=event).encode("utf8")
            logging.debug("Node {name}: fire event {event}"
                          .format(name=self.id, event=str(prefix[:-1])))
            self.pub_socket.send(prefix + msg.SerializeToString(), zmq.DONTWAIT)
        except Exception:
            logging.warning("Node {name}:could not send message exception"
                            .format(name=self.id))
            ...
            for handler in self.handlers.values():
                for event in handler.connected_events:
                    if event.node_id != input_node_id:
                        continue
                    self.event2handler[
                        _Event(event=event.declaration.name,
                               source_id=event.node_id)] = handler.declaration.name
                    ...
                    logging.debug(
                        "Node {id} subscriber addr={addr}, topic={topic}"
                        .format(id=self.id, addr=addr, topic=topic))
    ...
    def process_events_from_server(self):
        while not self.stopped:
            ...
            break
        logging.debug("Node {name}:process_events_from_server finished"
                      .format(name=self.id))
    ...
    def process_events(self):
        ...
        while not self.stopped:
            try:
                logging.debug("Node {name}: waiting for the next event..."
                              .format(name=self.id))
                ...
                handler_name = self.event2handler[_Event(source_id=source_id, event=event)] # noqa
                logging.debug("Node {name}: received event {event}"
                              .format(name=self.id, event=event))
    ...
    def execute(self):
        try:
            ...
            logging.debug("Node {id}. Publisher connected to port={port}"
                          .format(id=self.id, port=self.pub_port))
            ...
            logging.debug("Node {id}. Service connected to port={port}"
                          .format(id=self.id, port=self.service_port))
            ...
            self.send_command(json.dumps({
                ...
                "service_endpoint": "tcp://127.0.0.1:{port}"
                .format(port=self.service_port)}, sort_keys=True))
